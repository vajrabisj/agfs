...
#define SUMMARYFS_VERSION "0.1.0"
...
static const char *ensure_endpoint(SummaryFS *fs) {
    if (!fs->endpoint) {
        fs->endpoint = dup_string("http://localhost:11434/api/generate");
    }
...
static const char *call_ollama(SummaryFS *fs, const char *prompt) {
    const char *err;
...
static const char *extract_summary(SummaryFS *fs) {
    ...
    yyjson_val *resp = yyjson_obj_get(root, "response");
    const char *summary = resp ? yyjson_get_str(resp) : NULL;
    if (!summary) summary = "(no response field returned)";
...
static const char *apply_config(SummaryFS *fs, const char *config_json) {
    ...
    yyjson_val *endpoint_val = yyjson_obj_get(root, "ollama_url");
    ...
    if (endpoint_val && yyjson_is_str(endpoint_val)) {
        free_string(&fs->endpoint);
        const char *base = yyjson_get_str(endpoint_val);
        if (strstr(base, "/api/generate") == NULL) {
            size_t len = strlen(base) + strlen("/api/generate") + 2;
            char *buf = (char *)malloc(len);
            if (buf) {
                snprintf(buf, len, "%s/api/generate", base);
                fs->endpoint = buf;
            }
        } else {
            fs->endpoint = dup_string(base);
        }
    }
...
const char* PluginGetReadme(void* plugin) {
    (void)plugin;
    return "# SummaryFS\n"
           "Summarize arbitrary text via local Ollama (default model qwen3:4b).\n\n"
           "## Files\n"
           "- /request (write JSON: {\"text\":\"...\"})\n"
           "- /response.json (raw Ollama response)\n"
           "- /response.txt (plain summary)\n\n"
           "Config options: model, ollama_url, timeout_ms, system_prompt.\n";
}
...
static const char *handle_request(SummaryFS *fs, const char *data, int len) {
    ...
    const char *err = call_ollama(fs, prompt);
    free(prompt);
    if (err) return err;
